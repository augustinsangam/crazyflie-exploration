...
int exploration::WallFollowing::wall_follower(float *vel_x, float *vel_y,
  ...
  if (state_ == 1) { // FORWARD
    ...
    if (side_range_check && front_range_check) {
      previous_heading = current_heading;
      angle =
          direction_ * (1.57f - (float)atan(front_range / side_range) + 0.1f);
  ...
  if (state_ == 1) { // FORWARD
    ...
    if (side_range > ref_distance_from_wall_ + 0.5f) {
      ...
      if (around_corner_go_back) {
        ...
        command_turn_and_adjust(&temp_vel_y, &temp_vel_w, -1 * max_rate_,
                                side_range);
        temp_vel_x = 0.0f;
      } else {
    ...
    command_align_corner(&temp_vel_y, &temp_vel_w, -1 * max_rate_, side_range,
