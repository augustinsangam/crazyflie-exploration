...
static void commandAlignCorner(float *vel_y, float *vel_w, float ref_rate,
...
static void commandTurnAroundCornerAndAdjust(float *vel_x, float *vel_y,
                                             float *vel_w, float radius,
                                             float range) {
  *vel_x = max_speed;
  *vel_w = direction * (-1 * (*vel_x) / radius);
  bool check_distance_to_wall =
      logicIsCloseTo(ref_distance_from_wall, range, 0.1);
...
static void commandTurnAndAdjust(float *vel_y, float *vel_w, float rate,
                                 float range) {
...
int wall_follower(float *vel_x, float *vel_y, float *vel_w, float front_range,
  ...
  float now = sgba::usecTimestamp() / 1e6;
  ...
  if (state == 1) { // FORWARD
    ...
    bool side_range_check =
        side_range < ref_distance_from_wall / (float)cos(0.78f) + 0.2f;
    bool front_range_check =
        front_range < ref_distance_from_wall / (float)cos(0.78f) + 0.2f;
    if (side_range_check && front_range_check) {
      previous_heading = current_heading;
      angle =
          direction * (1.57f - (float)atan(front_range / side_range) + 0.1f);
    ...
    bool allign_wall_check = logicIsCloseTo(
        wraptopi(current_heading - previous_heading), angle, 0.1f);
    ...
    bool check_heading_corner = logicIsCloseTo(
        fabs(wraptopi(current_heading - previous_heading)), 0.8f, 0.1f);
  ...
  if (state == 1) { // FORWARD
    ...
    // commandForwardAlongWallHeadingSine(&temp_vel_x, &temp_vel_y,&temp_vel_w,
    // side_range);
    ...
    if (side_range > ref_distance_from_wall + 0.5f) {
      ...
      if (around_corner_go_back) {
        ...
        commandTurnAndAdjust(&temp_vel_y, &temp_vel_w, -1 * max_rate,
                             side_range);
        temp_vel_x = 0.0f;
      } else {
    ...
    commandAlignCorner(&temp_vel_y, &temp_vel_w, -1 * max_rate, side_range,
