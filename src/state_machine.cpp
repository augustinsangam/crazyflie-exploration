...
void exploration::StateMachine::iteration_loop() {
	...
	while (1) {
		...
		rssi_angle_inter_closest =
		    rssi_angle_array_other_drones[id_inter_closest];

		rssi_inter_filtered = (uint8_t)update_median_filter_f(
		    &medFilt_2, (float)rssi_inter_closest);
		...
		if (keep_flying) {
			if (taken_off) {
				...
				state = exploration_controller_.wall_follower(
				    &vel_x_cmd, &vel_y_cmd, &vel_w_cmd, front_range,
				...
				state =
				    exploration_controller_.wall_follower_and_avoid_controller(
				        &vel_x_cmd, &vel_y_cmd, &vel_w_cmd, front_range,
				        left_range, right_range, heading_rad,
				        rssi_inter_filtered);
#endif
				...
				state = exploration_controller_.sgba_controller(
				    ...
				    rssi_beacon_filtered, rssi_inter_filtered,
					...
					if (height > nominal_height) {
						...
						if (my_id % 2 == 1)
							exploration_controller_
							    .init_wall_follower_and_avoid_controller(
							        0.4, 0.5, -1);
						else
							exploration_controller_
							    .init_wall_follower_and_avoid_controller(
						...
						if (my_id == 4 || my_id == 8) {
							exploration_controller_.init_sgba_controller(
							    0.4, 0.5, -0.8);
...
#endif
		porting::commander_set_setpoint(&setpoint_BG,
		                                STATE_MACHINE_COMMANDER_PRI);
	}
}
