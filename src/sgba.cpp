...
int sgba::Sgba::transition(int new_state) {
	state_start_time_ = static_cast<float>(sgba::us_timestamp()) / 1e6F;
...
void sgba::Sgba::init_sgba_controller(float new_ref_distance_from_wall,
                                      float max_speed_ref,
...
int sgba::Sgba::sgba_controller(float *vel_x, float *vel_y, float *vel_w,
                                ...
                                float current_pos_y, std::uint8_t rssi_beacon,
                                std::uint8_t rssi_inter, float rssi_angle_inter,
	...
	if (first_run_) {
		...
		state_start_time_ = sgba::us_timestamp() / 1e6;
		first_run_ = false;
	}
